...
function Redimensionar($urlOrigen, $destino, $ancho){
		...
		if($image){
			...
			if($image){
				...
				if($image){
					...
					$image = Core_UtilsPicture::ImageCreateFromBMP($urlOrigen);
					if($image){
						$tipo = "bmp";
					}
				}
	...
	if ($tipo == "image/jpeg") imagejpeg($image_resized, $ruta_peque);
					...
					else if($tipo == "image/x-ms-bmp") imagewbmp($image_resized, $ruta_peque);
						else if($tipo == "bmp") imagejpeg($image_resized, $ruta_peque);
